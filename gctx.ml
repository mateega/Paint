...
type gctx = {
  ...
  size: size;   (** the current pen size *)
  }
...
let with_size (g: gctx) (s: size) : gctx =
  { g with size = s }
...
let draw_points (g: gctx) (pList: position list) : unit =
  set_graphics_state g;
  let draw_point_partial = draw_point g in
  List.iter draw_point_partial pList
